...
배경은 흰색(히어로 영역) → Gray 100(상세 섹션 영역). 탑바는 `close` 타입.
...
1. 히어로 (px-16, pb-16)
   - 헤드라인 2줄(heading-sub, Gray 950):
     "{근거수}개의 약한 근거가 있어요" — "{근거수}개"만 Purple 700
     "점검 결과를 확인해보세요"
   - 탭바: 점검 요약 / 약한 근거 / 객관적 자료 / 셀프 체크 질문 (4개, 스크롤 앵커, `sticky top-0` — 스크롤해서 탭이 화면 상단에 닿으면 고정되고, 다시 위로 스크롤해 원래 위치로 돌아오면 고정 해제)
   - 판단 요약 카드 (gradient-calm-subtle 배경, 1px Gray 200 보더, radius 16, padding 20, `w-full`)
     · 타이틀: 체크 아이콘 + "판단을 요약했어요" (그라데이션 텍스트, label 16px)
     · 헤드라인 2줄(label 16px semibold, Gray 800)
     · 설명 문단(label-sm 14px medium, Gray 700) — 실제 AI 응답 summary 바인딩

2. 상세 섹션 (Gray 100 배경, 흰 카드 3개, gap 16, padding 20 각)
   - 카드 공통: 타이틀 label-lg(20px semibold) + 아이콘 높이 20px(비율 유지), 타이틀↔불릿(또는 서브텍스트) gap 24px. 서브텍스트가 있는 카드는 타이틀↔서브텍스트 gap 12px, 서브텍스트 label-sm(14px)
   - 리스트 아이템(불릿) 공통: 불릿끼리 gap 16px, 주장 문장 label(16px semibold, Gray 800) + 설명 label-sm(14px, Gray 700), 주장↔설명 gap 8px

   a. 약한 근거 {근거수}개 — "{근거수}개"만 Purple 700, 아이콘❗
      리스트 아이템(반복, {근거수}만큼) → {근거}, {근거수} placeholder는 실제 weak_points 응답 데이터로 바인딩

   b. 메타인지 가동 자료(객관적 자료) — 아이콘🧠 + 서브텍스트
      동일한 리스트 아이템 포맷 → newsConnection/객관적 자료 데이터 바인딩

   c. 스스로 점검할 질문 — 아이콘🤔 + 서브텍스트
...
**정보 위계 원칙 (관찰 기반):**
- 판단 요약은 유일하게 그라데이션 배경 + 보더로 시각적 우선순위를 갖는다 — 나머지 3개 상세 섹션은 흰 카드로 동일한 무게를 갖되, Gray 100 배경 위에 나열되어 요약 카드보다는 한 단계 낮은 위계로 읽힌다
- 4개 탭은 균등한 접근성을 제공하지만(스크롤 앵커 + sticky), 화면 진입 시 항상 "요약"이 최상단에 먼저 보이도록 순서를 고정한다
